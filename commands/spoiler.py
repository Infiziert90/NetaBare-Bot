...
async def check_message(message):
    if not message.attachments:
        await send_message(message.author, "Picture as attachment is needed.")
    elif not message.attachments[0].width:
        await send_message(message.author, "Filetype is not allowed!")
    elif message.attachments[0].width * message.attachments[0].height > 8300000:
        await send_message(message.author, "Picture is too big.")
    elif message.attachments[0].width < 100:
        await send_message(message.author, "Picture width is too small.")
